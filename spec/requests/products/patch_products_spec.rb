RSpec.describe 'PATCH /api/v1/products/:id', type: :request do
  ...
  describe 'valid parameters' do
    ...
    it_should_behave_like 'updates value', 'name', 'Modified Name'

    it_should_behave_like 'updates value', 'price', '4.44'

    it_should_behave_like 'updates value', 'description', 'Modified Desc'

    context 'existing tags' do
      before do
        titles = create_list(:tag, 20).map { |tag| tag.title }
  ...
  describe 'invalid parameters' do
    ...
    context 'atomicity' do
      before { params[:data][:attributes].merge!(name: 'New Name', price: 4.44, description: 'New Desc', tags: generate_list(:title, 20)) }
